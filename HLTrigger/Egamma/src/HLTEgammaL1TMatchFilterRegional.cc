...
HLTEgammaL1TMatchFilterRegional::HLTEgammaL1TMatchFilterRegional(const edm::ParameterSet& iConfig) : HLTFilter(iConfig)
{
   ...
   candEgTag_ = iConfig.getParameter< edm::InputTag > ("candEgTag");
   l1EgTag_ = iConfig.getParameter< edm::InputTag > ("l1EgTag"); 
   ...
   candEgToken_ = consumes<reco::RecoEcalCandidateCollection>(candEgTag_);
...
HLTEgammaL1TMatchFilterRegional::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
  ...
  desc.add<edm::InputTag>("candEgTag",edm::InputTag("hltRecoEcalCandidate"));
  desc.add<edm::InputTag>("l1EgTag",edm::InputTag("l1tEgamma","Egamma"));
  ...
  desc.add<edm::InputTag>("l1CenJetsTag",edm::InputTag("l1tJet","Central"));
...
HLTEgammaL1TMatchFilterRegional::hltFilter(edm::Event& iEvent, const edm::EventSetup& iSetup, trigger::TriggerFilterObjectWithRefs & filterproduct) const
{
  ...
  iEvent.getByToken(candEgToken_,recoEcalcands);
  ...
  l1t::EGammaVectorRef l1EG;
  L1SeedOutput->getObjects(TriggerL1EG, l1EG);
  ...
  for (reco::RecoEcalCandidateCollection::const_iterator recoecalcand= recoEcalcands->begin(); recoecalcand!=recoEcalcands->end(); recoecalcand++) {
    ...
    if(fabs(recoecalcand->eta()) < endcap_end_){
      ...
      bool matchedSC = matchedToL1Cand(l1EG,recoecalcand->eta(),recoecalcand->phi());
	...
	//matchedSCNonIso =  matchedToL1Cand(l1EGNonIso,recoecalcand->eta(),recoecalcand->phi());
      ...
      if(matchedSC || matchedSCJet) {
	n++;

//	ref = edm::Ref<reco::RecoEcalCandidateCollection>(recoIsolecalcands, distance(recoIsolecalcands->begin(),recoecalcand) );
	ref = edm::Ref<reco::RecoEcalCandidateCollection>(recoEcalcands, distance(recoEcalcands->begin(),recoecalcand) );
...
HLTEgammaL1TMatchFilterRegional::matchedToL1Cand(const l1t::JetVectorRef l1Cands,const float scEta,const float scPhi) const
{
  for (unsigned int i=0; i<l1Cands.size(); i++) {
    //ORCA matching method
    double etaBinLow  = 0.;
