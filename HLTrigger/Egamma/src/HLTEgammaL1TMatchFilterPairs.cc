...
#include "HLTrigger/Egamma/interface/HLTEgammaL1TMatchFilterPairs.h"
...
//
HLTEgammaL1TMatchFilterPairs::HLTEgammaL1TMatchFilterPairs(const edm::ParameterSet& iConfig) : HLTFilter(iConfig)
{
...
HLTEgammaL1TMatchFilterPairs::~HLTEgammaL1TMatchFilterPairs(){}

void
HLTEgammaL1TMatchFilterPairs::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
  ...
  desc.add<bool>("trigger_bx_only",true);
  descriptions.add("HLTEgammaL1TMatchFilterPairs",desc);
}
...
bool
HLTEgammaL1TMatchFilterPairs::hltFilter(edm::Event& iEvent, const edm::EventSetup& iSetup, trigger::TriggerFilterObjectWithRefs & filterproduct) const
{
...
//bool HLTEgammaL1TMatchFilterPairs::CheckL1Matching(edm::Ref<reco::RecoEcalCandidateCollection> ref, std::vector<l1extra::L1EmParticleRef >& l1EGIso, std::vector<l1extra::L1EmParticleRef >& l1EGNonIso) const {
bool HLTEgammaL1TMatchFilterPairs::CheckL1Matching(edm::Ref<reco::RecoEcalCandidateCollection> ref, l1t::EGammaVectorRef l1EGIso, l1t::EGammaVectorRef l1EGNonIso) const {

  for (unsigned int i=0; i<l1EGIso.size(); i++) {
    double hltEta = l1EGIso[i]->eta();
    double hltPhi = l1EGIso[i]->phi();
    //double hltEta = -999.;
    //double hltPhi = -999.;
    //for (int ibx = l1EGIso[i]->getFirstBX(); ibx <= l1EGIso[i]->getLastBX(); ++ibx) {
    //  if (trigger_bx_only && (ibx != 0)) continue; 
    //    for (auto it=l1EGIso[i]->begin(ibx); it!=l1EGIso[i]->end(ibx); it++){
    //    hltEta = it -> eta();
    //    hltPhi = it -> phi();
  ...
  for (unsigned int i=0; i<l1EGNonIso.size(); i++) {
    double hltEta = l1EGNonIso[i]->eta();
    double hltPhi = l1EGNonIso[i]->phi();
    //double hltEta = -999.;
    //double hltPhi = -999.;
    //for (int ibx = l1EGNonIso[i]->getFirstBX(); ibx <= l1EGNonIso[i]->getLastBX(); ++ibx) {
    //  if (trigger_bx_only && (ibx != 0)) continue; 
    //  for (auto it=l1EGNonIso[i]->begin(ibx); it!=l1EGNonIso[i]->end(ibx); it++){
    //    hltEta = it -> eta();
    //    hltPhi = it -> phi();
